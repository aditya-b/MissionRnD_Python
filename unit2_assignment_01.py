...
def custom_sort(input):
    if type(input).__name__=="NoneType":
        return None
    elif input.__len__()==0:
        return []
    else:
        input.sort(key=lambda x:x[2])
        input.sort(key=lambda y:y[1],reverse=True)
        return input
...
def max_vowels(words):
    if type(words).__name__=='NoneType':
        return None
    elif words.__len__()==0:
        return None
    else:
        max=0
        word=words[0]
        for i in words:
            count=(set(i)&VOWELS).__len__()
            if count>max:
                max=count
                word=i
        return word
